...
public class MultiStack {
	...
	private class StackInfo {                                                                  // Stack info about particular stack
	...
	private StackInfo[] info;                      //Stack array
	private int[] values;                          // whole value array

	public MultiStack(int numberOfStacks, int defaultSize) {               //Constructor for MultiStack
	...
	public int numberOfElements() {                                                        // NUmber of elements in whole stack
		int size = 0;
	...
	public boolean allStacksAreFull() {                                                    // CHeck if all stack are full
		return numberOfElements() == values.length;
	}

	/* Adjust index to be within the range of 0 -> length - 1. */
	private int adjustIndex(int index) {                                                   // Adjust Index
		...
		return ((index % max) + max) % max;                                               // Important
	}

	/* Get index after this index, adjusted for wrap around. */
	private int nextIndex(int index) {                                                    // Next index
		return adjustIndex(index + 1);
	}

	/* Get index before this index, adjusted for wrap around. */
	private int previousIndex(int index) {                                                // Prev index
		return adjustIndex(index - 1);
	}
